package exercicio2_test1poo;

// * @author Pacule, Manuel Arnaldo
...
public class Menu {
    ...
    public Menu() {
        ...
        int opcao;
        char resp = ' ';
        System.out.println("BEM BINDO A Lanchonete do Pacule");
        ...
        do {
            System.out.println("""
                              ...
                              |    4. Sair         | 
                               -------------------- 
                               """);
            opcao = vd.validarInt(1, 4, "Faca o seu pedido: ");
    ...
    public float pedidoPizza() {
        int quant = vd.validarInt(1, 10, "\nDeseja comprar quantas pizzas: ");
        ArrayPizza = new Pizza[quant];
        float totalPizza = 0;

        for (int i = 0; i < ArrayPizza.length; i++) {
            float peso = vd.validarFloat(20, 150, "Introduza o peso da " + (i + 1) + "ª a pizza: ");
            String recheio = ChooseRecheio("pizza");
    ...
    public float pedidoLanche() {
        ...
        ArrayLanche = new Lanche[quant];
        float totalLanche = 0;

        for (int i = 0; i < ArrayLanche.length; i++) {
            float peso = vd.validarFloat(20, 150, "Introduza o peso do " + (i + 1) + "º o lanche: ");
            String recheio = ChooseRecheio("lanche");
            String molho = vd.validarString("Qual molho deseja: ");
            ...
            try {
                FileWriter fw = new FileWriter("Lanche.txt", true);
                BufferedWriter bw = new BufferedWriter(fw);
                bw.write(peso + ";" + preco + ";" + 7 + ";" + recheio + ";" + molho + ";" + tipoPao);
                bw.newLine();
                bw.close();
            } catch (IOException io) {
                System.out.println("error: " + io.getMessage());
    ...
    public float pedidoSalgadinho() {
        ...
        ArraySalgadinho = new Salgadinho[quant];
        float totalSalgadinho = 0;

        for (int i = 0; i < ArraySalgadinho.length; i++) {
            float peso = vd.validarFloat(20, 150, "Introduza o peso do " + (i + 1) + "º o salgadinho: ");
            String recheio = ChooseRecheio("salgadinho");
            String massa = vd.validarString("Qual massa deseja: ");
            String tipo = ChooseTypeSalgadinho();
            float preco = 100 * peso / 100;
            totalSalgadinho += preco;

            ArraySalgadinho[i] = new Salgadinho(peso, recheio, massa, tipo, preco);
            System.out.println("");

            try {
                FileWriter fw = new FileWriter("Salgadinho.txt", true);
                BufferedWriter bw = new BufferedWriter(fw);
                bw.write(peso + ";" + preco + ";" + 7 + ";" + recheio + ";" + massa + ";" + tipo);
                bw.newLine();
                bw.close();
            } catch (IOException io) {
                System.out.println("error: " + io.getMessage());
    ...
    public String ChooseRecheio(String item) {
        String tipoRecheio;
        System.out.println("""
    ...
    public String ChooseTypeSalgadinho() {
